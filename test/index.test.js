...
const fingerrootBridgeInstanceStub = {
    _address: '0x0000000000000000000000000000000001000006',
    _jsonInterface: [
        {
            name: 'updateCollections',
...
const fingerrootBridgeStub = {
    build: () => {
        return fingerrootBridgeInstanceStub
    },
    address: '0x0000000000000000000000000000000001000006',
    abi: [
        {
...
const postFingerrootTxReceiptsStub = [
    // Non Bridge transactions
    {
        transactionHash: '0x6547e88a30d1b43c6fbea07fa7443dfeb697d076495c3e4fc56ebf40228e0431',
        blockNumber: 1001,
...
const postFingerrootWeb3ClientStub = {
    eth: ({
        getTransactionReceipt: (txHash) => {
            return postFingerrootTxReceiptsStub.find(txReceipt => txReceipt.transactionHash === txHash);
        },
        getTransaction: (txHash) => {
            return transactionsStub.find(tx => tx.hash === txHash);
        },
...
describe('Get Bridge Transaction By Tx Hash', () => {
    ...
    beforeEach((done) => {
        ...
        transactionParser.__set__({
            'HopBridge': hopBridgeStub
        });
        transactionParser.__set__({
            'FingerrootBridge': fingerrootBridgeStub
        });
...
describe('Gets a Bridge Transaction given a bridgeTx: web3TransactionObject and a bridgeTxReceipt: TransactionReceipt', () => {
    ...
    it(`Should decode a bridge transaction from a bridgeTx web3Object and a bridgeTxReceipt as well as a web3Object'
      'into a Transaction object with the release_request_received signature as of fingerroot`, async () => {
        const bridgeTx = postFingerrootWeb3ClientStub.eth.getTransaction('0x73a4d1592c5e922c2c6820985982d2715538717e4b4b52502685bc4c924300b7');
        const bridgeTxReceipt = postFingerrootWeb3ClientStub.eth.getTransactionReceipt('0x73a4d1592c5e922c2c6820985982d2715538717e4b4b52502685bc4c924300b7');

        const transaction = await transactionParser.decodeBridgeTransaction(postFingerrootWeb3ClientStub, bridgeTx, bridgeTxReceipt, true)
